...
export const fetchYoutubeVideosWithoutApiKey = async (playlistId: string): Promise<Video[]> => {
	try {
	const response = await fetch(`https://yt.lemnoslife.com/noKey/playlistItems?part=snippet&playlistId=${playlistId}`);
	if (!response.ok) {
...
export const fetchYoutubeVideosWithApiKey = async (playlistId: string): Promise<Video[]> => {
	try {
		const apiKey = process.env.EXPO_PUBLIC_YOUTUBE_API_KEY;
		if (!apiKey) {
		throw new Error('YouTube API key is required (EXPO_PUBLIC_YOUTUBE_API_KEY)');
	}
...
type YtPlaylistItem = { snippet?: YtSnippet, contentDetails?: YtPlaylistContentDetails };
...
type YtLiveStreamingDetails = { actualStartTime?: string, scheduledStartTime?: string };
type YtVideo = {
	id?: string,
	snippet?: YtSnippet,
...
export const fetchPlaylistVideos = async (
	...
	const itemsJson = (await itemsRes.json()) as unknown as YtPlaylistItemsResponse;
