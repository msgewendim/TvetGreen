...
function generateWithImageMagick(source, sizes) {
	sizes.forEach(({ size, output }) => {
		...
		try {
			execSync(
				`convert "${source}" -resize ${size}x${size} -background none -gravity center -extent ${size}x${size} "${outputPath}"`,
				{
					stdio: "inherit",
				},
			);
...
function fallbackCopy(source, sizes) {
	...
	console.warn(
		"⚠ Icons may not be the correct size. Please resize manually or install an image tool.",
	);
	...
	sizes.forEach(({ size, output }) => {
		...
		if (fs.existsSync(source)) {
			...
			console.log(
				`✓ Copied ${output} (may need manual resizing to ${size}x${size})`,
			);
		}
	});
}
